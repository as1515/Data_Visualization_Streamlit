...
def visualize_histogram(data, selected_column, selected_metric):
    ...
    bin_counts_df = pd.DataFrame({"Bin Range": bin_counts.index.astype(str),"Count": bin_counts.values})
        # Remove the `[` and `)` from the Bin Range column
    bin_counts_df['Bin Range'] = bin_counts_df['Bin Range'].str.replace('[', '').str.replace(')', '')
    ...
    st.write(bin_counts_df, width=600, use_container_width=True)
