...
def net_pivot(data1,data2, params1,current_page):
    ...
    try:
        ...
        index_cols = params1['index'] if isinstance(params1['index'], list) else [params1['index']]
        
        # Generate pivot tables
        pivot1 = data1.pivot_table(
            values=params1['valuesales'],
            index=index_cols, 
            columns=params1['column'], 
            aggfunc='sum'
        ).fillna(0)
        
        # Handle case when returns data is empty
        if data2.empty:
            pivot2 = pd.DataFrame(
                0, 
                index=pivot1.index, 
                columns=pivot1.columns
            )
        else:
            pivot2 = data2.pivot_table(
                values=params1['valuereturn'],
                index=index_cols, 
                columns=params1['column'], 
                aggfunc='sum'
            ).fillna(0)
        ...
        # Ensure both pivot tables have the same columns and indices
        all_columns = pivot1.columns.union(pivot2.columns)
        all_indices = pivot1.index.union(pivot2.index)
        
        pivot1 = pivot1.reindex(columns=all_columns, index=all_indices).fillna(0)
        pivot2 = pivot2.reindex(columns=all_columns, index=all_indices).fillna(0)
        
        # Perform operation based on current page
        ...
        # Safely sort columns
        try:
            # Modify sorting to handle float years
            if current_page == 'YOY Analysis':
                sorted_columns = sorted(pivot.columns, key=lambda x: (
                    months_order.index(x[1]) if x[1] in months_order else -1, 
                    int(float(x[0])) if isinstance(x[0], (int, float)) else -1
                ))
            else:
                sorted_columns = sorted(pivot.columns, key=lambda x: (
                    int(float(x[0])) if isinstance(x[0], (int, float)) else -1, 
            ...
            print(f"Column sorting error: {sort_error}")
        
        # Calculating the grand total
        pivot['Grand Total'] = pivot.sum(axis=1)
        pivot = pivot.sort_values(by='Grand Total', ascending=False)
        
        # Convert all decimal.Decimal columns to float and round
